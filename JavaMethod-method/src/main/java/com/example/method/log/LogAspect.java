...
public class LogAspect {
     ...
     * @annotation(log) 表示这个切入点匹配所有被 @log 注解标记的方法。这里的 @log 应该是一个自定义的注解，用于标识需要进行日志记录或其他操作的方法。
     * 使用场景
     * 假设你有一个自定义注解 @log，用于标记需要进行日志记录的方法。你可以创建一个切面类，并使用 @Around("@annotation(log)") 来定义一个环绕通知，这个通知会在所有被 @log 注解标记的方法执行前后执行。你可以在环绕通知中添加日志记录的代码，从而实现对这些方法的日志记录功能。
     */
    ...
    public Object logExecution(ProceedingJoinPoint joinPoint, Log log) throws Throwable {
        ...
        // 打印执行结束的信息
        System.out.println("执行结束：" + log.value());
